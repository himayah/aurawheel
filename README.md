...
* **インタラクティブ・ツールチップ**:
  * パレットに保存された色にマウスカーソルを重ねる（ホバーする）だけで、その色の **HEX、RGB、HSLカラーコード** がガラスモルフィズム調の美しいツールチップで表示されます。
  * **上下左右の見切れ防止機能**: 
    * 左右両端の列（左端2列・右端2列）にあるセルをホバーした際、ツールチップの吹き出しのアライメント（左寄せ・右寄せ）を動的に切り替え、コンテナ外にはみ出すのを完全に防ぎます。
    * 上部の3行にあるセルをホバーした際は下側に、下部のセルは上側に表示方向を自動判定して切り替えるため、上端・下端の見切れも発生しません。これにより、常にすべての情報（特に最上段のHEXコードなど）が隠れずに美しく表示されます。
